...
/* Note: Make sure the stdc++6 libs are included!                                 */
/* You can make the necessary configurations with petalinux-config -c rootfs!     */
/*--------------------------------------------------------------------------------*/
#include <iostream>
#include <stdint.h>
#include <stdexcept>
...
int main( int argc, char** argv )
{
	...
	if ( !( ( argc >= 2 ) && ( argc <= ( DISPLAY_NUM_FRAMES+1 ) ) ) )
		throw runtime_error( "usage: opencv_linux_0.elf <Image_Path> [<Image_Path> <Image_Path>]" );
	nframes = argc-1;
