...
#define GPIO_BASE_INS		894
#define GPIO_OUTS_NPINS		4
#define GPIO_INS_NPINS		8
...
/* The following functions simplify utilization of the GPIO driver. These
 functions were taken from a Xilinx example. */
...
/* Signal handler is needed to ensure gpio is closed
 properly when the program ends. */
...
int main(int argc, char *argv[])
{
	...
	while ( true )
	{
		...
		/* Read from GPIO. */
		ins = get_gpio_value( GPIO_BASE_INS, GPIO_INS_NPINS );
		if ( ins < 0 )
		{
